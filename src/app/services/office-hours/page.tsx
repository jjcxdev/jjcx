import { Metadata } from "next";

export const metadata: Metadata = {
  title: "Office Hours — JJCX Inc.",
  description:
    "60-minute paid advisory session. Bring a specific annotation ops or HITL workflow problem and leave with a concrete direction.",
};

export default function OfficeHoursPage() {
  return (
    <>
      <nav>
        <div className="nav-inner">
          <a className="nav-logo" href="/">
            <svg viewBox="0 0 1558 485" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ height: "28px", width: "auto", display: "block" }}>
              <path d="M7.65839 397.369L74.8308 311.868H124.233C184.335 311.868 209.269 302.099 209.269 270.002V99.5587H312.539V262.001C312.539 352.339 249.74 397.369 124.233 397.369H7.65839Z" fill="#00878A" />
              <path d="M341.474 397.369L408.646 311.868H458.049C518.151 311.868 543.084 302.099 543.084 270.002V99.5587H646.355V262.001C646.355 352.339 583.555 397.369 458.049 397.369H341.474Z" fill="#00878A" />
              <path d="M870.759 397.369C766.186 397.369 689.152 331.964 689.152 245.533C689.152 158.265 766.186 99.5587 870.759 99.5587H1084.46L1018.59 186.362H870.759C823.125 186.362 792.422 201.527 792.422 245.533C792.422 290.005 823.125 311.868 870.759 311.868H1084.46L1018.59 397.369H870.759Z" fill="#00878A" />
              <path d="M1409.81 397.369L1319.01 305.635L1224.67 397.369H1096.47L1190.43 308.798C1237.6 264.327 1252.77 252.697 1259.84 247.859C1252.67 242.091 1237.6 230.926 1196.67 191.293L1100.93 99.5587H1243.84L1329.34 186.362L1415.67 99.5587H1548.81L1449.08 192.595C1414.84 224.6 1400.14 236.695 1390.74 243.766C1403.67 252.232 1422.37 268.234 1464.15 309.635L1554.95 397.369H1409.81Z" fill="#00878A" />
            </svg>
          </a>
          <ul className="nav-links">
            <li><a href="/#services">Services</a></li>
            <li><a href="/work">Case Studies</a></li>
            <li><a href="/articles">Articles</a></li>
            <li><a href="/#about">About</a></li>
            <li><a href="/#contact" className="nav-cta">Get in touch</a></li>
          </ul>
        </div>
      </nav>

      <div className="article-page">
        <div className="container">
          <div className="article-content">

            <h1 className="article-title">Office <span className="article-title-accent">Hours</span></h1>

            <p>
              60 minutes. One specific problem. A concrete direction forward.
            </p>

            <h2>What this is</h2>
            <p>
              A paid advisory call with someone who runs annotation programs for a living.
              No retainer, no ongoing commitment. Bring a specific problem and leave with
              a clear next step.
            </p>

            <h2>Good problems to bring</h2>
            <p>
              Annotation quality is stuck and you can&apos;t figure out why. Your QA process
              exists on paper but isn&apos;t catching real issues. You&apos;re building a HITL
              workflow from scratch and want to avoid the common mistakes. Annotator
              performance is inconsistent across your team. Your scoring methodology
              might be masking the actual problem. You&apos;re scaling an annotation program
              and the wheels are starting to come off.
            </p>

            <h2>How it works</h2>
            <p>
              Book and pay below. You&apos;ll receive a confirmation email. Reply with
              three times that work for you and a brief description of what you want
              to cover. I&apos;ll confirm a slot and send a video call link.
            </p>
            <p>
              Come prepared with specifics. The more context you share upfront, the
              more useful the session will be. Screenshots, metrics, workflow diagrams,
              guideline docs — anything that helps me understand your setup before we talk.
            </p>

            <hr />

            <div className="pricing-block">
              <p className="pricing-amount">$500 <span className="pricing-currency">CAD</span></p>
              <p className="pricing-detail">60-minute video call &nbsp;·&nbsp; No retainer required</p>
              <a href="https://buy.stripe.com/aFabJ2952dw8aTecceeUU01" target="_blank" rel="noopener noreferrer" className="btn-primary">Book a session</a>
              <p className="pricing-footnote">
                Not sure if this is the right fit? <a href="mailto:j@jjcx.dev">Send me a note.</a>
              </p>
            </div>

          </div>
        </div>
      </div>

      <footer>
        <div className="footer-inner">
          <span className="footer-left">&copy; 2026 JJCX Inc.</span>
          <div className="footer-right">
            <a href="/privacy">Privacy</a>
            <a href="/terms">Terms</a>
            <a href="https://linkedin.com/in/justinjchambers" target="_blank" rel="noopener noreferrer">LinkedIn</a>
            <a href="mailto:j@jjcx.dev">Contact</a>
          </div>
        </div>
      </footer>
    </>
  );
}
